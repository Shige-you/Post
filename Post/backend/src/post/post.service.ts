...
export class PostService {
  ...
  async getList(token: string, start: number = 0, nr_records: number = 1) {
    ...
    console.log("取得データ",records);
    return records;
  }

  async deletePost(id:number,token:string){
    return await this.microPostsRepository.delete({id:id});
  }

  async updatePost(id:number,content:string,token:string){
    return await this.microPostsRepository.update(id,{content:content});
  }
}
